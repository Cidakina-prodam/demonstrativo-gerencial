...
with st.spinner("Gerando HTML..."):
    ...
    # ── OPERAÇÃO ASSISTIDA ────────────────────────────────────────────────────
    # Filtrar lançamentos de Operação Assistida no período
    # Filtro OA: busca "Assistida" em tipo_demanda e ordem_servico (robusto a encoding)
    ...
    for nucleo in NUCLEOS:
        ...
        for _, cr in cn.iterrows():
            ...
            total_oa += h_oa
            # Agrupar por projeto > GDP > atividade
            projs = []
    ...
    dados_js = (
        ...
        "var OA="      + json.dumps(OA_MES,            ensure_ascii=True) + ";\n"
