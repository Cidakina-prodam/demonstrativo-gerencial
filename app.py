...
if "_mes_ant" not in st.session_state:
    st.session_state._mes_ant = None

# ── SIDEBAR ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## ⚙️ Configuração")
    mes_ref = st.selectbox("Mês de Referência", list(PERIODOS.keys()), index=4)
    if st.session_state._mes_ant not in (None, mes_ref):
        st.session_state.pdf_key += 1          # força reset do uploader de PDFs
    st.session_state._mes_ant = mes_ref
    ...
    pdf_files   = st.file_uploader("PDFs Posicionais", type=["pdf"],
                                    ...
                                    key=f"pdf_{st.session_state.pdf_key}")
# ── VERIFICAR UPLOADS ─────────────────────────────────────────────────────────
...
with st.spinner("Gerando HTML..."):
    ...
    eh_oa = (
        (_td.str.contains("ASSISTIDA", na=False) | _os.str.contains("ASSISTIDA", na=False))
        & ~periodo["eh_prodam"]   # excluir lançamentos internos PRODAM (treinamentos, etc.)
    )
    ...
    # POS histórico indexado por mês: {"Abril 2026": {nss1: ...}, ...}
    pos_hist_raw = dados_hist.get("pos") or {}
    # Compatibilidade: detectar formato antigo (flat {nss1: {...secretarias...}})
    # vs novo ({"Abril 2026": {nss1: ...}})
    # No formato antigo, as chaves são nomes de núcleo (nss1, nss2, nss3, nc)
    NUCLEO_KEYS = {"nss1", "nss2", "nss3", "nc"}
    is_formato_antigo = bool(pos_hist_raw) and bool(NUCLEO_KEYS & set(pos_hist_raw.keys()))
    if is_formato_antigo:
        # Descarta histórico antigo — não é possível saber a qual mês pertencia
        pos_final = {}
    else:
        pos_final = dict(pos_hist_raw)
    # Remover meses excluídos do POS histórico
    for m in meses_remover:
        pos_final.pop(m, None)
    # Adicionar mês atual
    if pos_json_dict:
        pos_final[mes_ref] = pos_json_dict

    lanc_final = dict(dados_hist.get("lanc") or {})
    lanc_final[mes_ref] = lanc_por_nucleo
    for m in meses_remover:
        lanc_final.pop(m, None)
