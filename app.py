...
with st.spinner("Gerando HTML..."):
    ...
    # Compatibilidade: se ainda é formato antigo (flat {nss1: ...}), ignora
    if pos_hist_raw and not any(isinstance(v, dict) and "label" not in v and
                                 any(k2 in v for k2 in ("secretarias","label")) 
                                 for k2, v in pos_hist_raw.items()
                                 if isinstance(v, dict)):
        pos_final = dict(pos_hist_raw)  # pode ser formato antigo ou novo
    else:
