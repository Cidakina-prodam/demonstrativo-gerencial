...
def processar_posicional(
    ...
    for sec_def in estrutura_pdf.get("secretarias", []):
            ...
            for proj_def in os_def.get("projetos", []):
                ...
                for dem_def in proj_def.get("demandas", []):
                    ...
                    if ativs:
                        ...
                        gdp_norm = str(gdp).replace(".0","").strip()
                        gds_norm = str(gds).replace(".0","").strip()
                        lancs_gdp = mapa_gdp.get(gdp_norm, [])
                        if not lancs_gdp and gds_norm and gds_norm != gdp_norm:
                            lancs_gdp = mapa_gdp.get(gds_norm, [])
                        ...
                        if not lancs_gdp_periodo:
                            ...
                            if lancs_gdp:
                                # Há histórico fora do período — não é divergência real
                                # Popula colaboradores com fora_periodo=True para exibir badge 📅
                                for l in lancs_gdp:
                                    rf = l["rf"]
                                    colaboradores[rf]["nome"] = l["nome"]
                                    colaboradores[rf]["rf"]   = rf
        ...
        # Recalcula o total da secretaria a partir dos projetos já somados,
        # em vez de confiar no valor bruto extraído do PDF (estrutura_pdf).
        # O cabeçalho "Qtdes. Realizadas" do PDF pode vir errado/incompleto
        # (ex.: refletindo só o primeiro OS) — a soma das linhas é a fonte
        # de verdade, igual já fazemos na Prévia de Faturamento.
        soma_real = round(
            sum(p["horas"] for os_ in sec_out["oss"] for p in os_["projetos"]), 2
        )
        sec_out["horas"] = soma_real
