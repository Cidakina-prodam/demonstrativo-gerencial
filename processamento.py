...
def processar_posicional(
    ...
    lanc_df["_in_per"] = in_per_mask
    lanc_df["_nome_fmt"] = lanc_df.apply(
        lambda r: san(str(r.get("nome", r["rf"]) or r["rf"])).title(), axis=1
    )
    lanc_df["_desc_atv"] = lanc_df["titulo_atividade"].fillna("").apply(lambda x: san(str(x)))
