            ...
            for proj_def in os_def.get("projetos", []):
                ...
                for dem_def in proj_def.get("demandas", []):
                    ...
                    if ativs:
                        ...
                        import sys
                        if gdp_norm in ("173276",) or gds_norm in ("25070",):
                            chaves = [k for k in mapa_gdp if "173" in k]
                            print(f"DEBUG_SEM_ATV gdp_norm={gdp_norm!r} gds_norm={gds_norm!r} lancs_gdp={len(lancs_gdp)} chaves_173x={chaves[:5]}", file=sys.stderr)
