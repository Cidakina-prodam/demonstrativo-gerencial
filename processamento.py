                ...
                for dem_def in proj_def.get("demandas", []):
                    ...
                    if ativs:
                        ...
                        for atv in ativs:
                            ...
                            # Fallback: se o ID de atividade não bater, buscar pelo GDP
                            # (ocorre quando posicional usa ID diferente do CSV, ex: Mantis vs OS)
                            if not lancs and gdp:
                                lancs_gdp_fb = mapa_gdp.get(str(gdp), [])
                                # Só usa o fallback se todos os IDs de atividade são diferentes
                                # (evita contaminar demandas com IDs corretos)
                                todos_ids_csv = {l["atv"] for l in lancs_gdp_fb}
                                if atv not in todos_ids_csv and todos_ids_csv:
                                    lancs = lancs_gdp_fb
                            # Faltando: nenhum lançamento no período atual
                            lancs_periodo = [l for l in lancs if l["no_periodo"]]
                            if not lancs_periodo:
                                # Marca divergência — atividade sem lançamento no período
                                faltando.append(atv)
                                # Mas exibe lançamentos históricos (fora do período)
                                # como contexto, marcados com fora_periodo=True
                                for l in lancs:
                                    rf = l["rf"]
